...
承接第一輪（`../2026-09-24-source-followup/`）剩餘的 215 件公告標題型國家缺口。本輪已處理 83 件：**27 件補入官方來源、10 件保留待確認、46 件查無可用官方來源或僅有國內紀錄**；國家待補證 215 → **188** 件。本機仍為 517 件。

- 第一階段（已上線，commit 64471a4）：24 件採用，待補證 → 191。
- 第二階段（其他運動項目）：再採用 3 件（Struxness、Colunga、Mazzie），另 4 件因同人疑義保留（Dumala 官方頁改列他姓、Burkett 與 Howe 名稱後綴不同、Harbert 官方頁為壘球）。
- 剩餘 188 件以舉重（約 66 件）與自行車（約 58 件）的 masters／國內選手為主，現行官方網站多無資料。

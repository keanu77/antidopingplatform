...
// /api/feedback 把 feedback / error 存成 type='issue'，細節以 feedback-v1 JSON 放在 message。
const V1_LABELS = {
  issueType: "問題類型",
  errorType: "錯誤類型",
  description: "問題描述",
...
function parseV1(message) {
  if (!message?.startsWith("{")) return null;
  try {
    const detail = JSON.parse(message);
    return detail?.格式 === "feedback-v1" ? detail : null;
  } catch {
    return null;
  }
}
...
/**
 * D1 列 → 彙整用欄位名。page 欄位存的就是前端的 toolSlug。
 * 優先解 feedback-v1 JSON；09-24 短暫上線的版本用的是 0002 的獨立欄位，一併相容。
 */
export function rowFromD1(r) {
  const base = { type: r.type, toolSlug: r.page || "/", rating: r.rating };
  const v1 = parseV1(r.message);
  if (v1) {
    const fields = Object.fromEntries(
      Object.entries(V1_LABELS).map(([key, label]) => [
        key,
        v1[label] ?? (key === "message" ? "" : null),
      ]),
    );
    return {
      ...base,
...
export function summarize(rows) {
  ...
  const messages = rows.filter(
    (r) => (r.type === "feedback" || r.type === "issue") && r.message,
  );

  const byTool = new Map();
  for (const r of ratings)
    byTool.set(r.toolSlug, [...(byTool.get(r.toolSlug) ?? []), r.rating]);

...
// MongoDB 連線一律讀環境變數，禁止硬編憑證。
// ⚠️ 本檔原本硬編了 production 連線字串（含帳密）——已移除。
//    該憑證已存在於 git 歷史（公開 repo），請務必在 Zeabur / MongoDB 端「輪換密碼」。
if (!process.env.MONGODB_URI) {
  console.error(
    '✗ 請先設定 MONGODB_URI 環境變數再執行（例：export MONGODB_URI="mongodb://.../sports-doping-db?authSource=admin"）。',
  );
  process.exit(1);
}
